...
public class PrimaveraApplicationContext {
    ...
    private void validateLifecycleMethod(Method method, String annotationName) {
        ...
        if (method.getParameterCount() > 0) {
            throw new RuntimeException(String.format("%s 메서드는 매개변수가 없어야 합니다: %s.%s", annotationName, method.getDeclaringClass().getSimpleName(), method.getName()));
        }

        // static이 아니어야 함
        if (java.lang.reflect.Modifier.isStatic(method.getModifiers())) {
            throw new RuntimeException(String.format("%s 메서드는 static이 아니어야 합니다: %s.%s", annotationName, method.getDeclaringClass().getSimpleName(), method.getName()));
        }

        // final이 아니어야 함
        if (java.lang.reflect.Modifier.isFinal(method.getModifiers())) {
            throw new RuntimeException(String.format("%s 메서드는 final이 아니어야 합니다: %s.%s", annotationName, method.getDeclaringClass().getSimpleName(), method.getName()));
        }
    }
