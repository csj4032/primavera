...
//@Disabled("Complex transaction isolation test - requires database and transaction management")
public class WinnerServiceIsolationTest {
    ...
    public void read_uncommitted_insert() {
        ...
        sqlSession = sqlSessionFactory.openSession();
        WinnerMapper winnerMapper = sqlSession.getMapper(WinnerMapper.class);
        winnerMapper.save(Winner.builder().id(1).userId(1).winner(WinnerType.LOSER).createdAt(Instant.now()).build());
        winnerMapper.save(Winner.builder().id(2).userId(2).winner(WinnerType.LOSER).createdAt(Instant.now()).build());
    }
    ...
    public void read_committed_repeatable() throws SQLException {
        ...
        localWinnerMapper.update(Winner.builder().id(1).winner(WinnerType.WINNER).createdAt(Instant.now()).build());
    ...
    public void repeatable_read_select() {
        ...
        SqlSession localSqlSession2 = sqlSessionFactory.openSession();
        WinnerMapper localWinnerMapper = localSqlSession2.getMapper(WinnerMapper.class);
        localWinnerMapper.update(Winner.builder().id(1).userId(1).winner(WinnerType.LOSER).createdAt(Instant.now()).build());
        transactionManager.commit(status2);
    ...
    public void repeatable_read_insert() {
        ...
        SqlSession localSqlSession2 = sqlSessionFactory.openSession();
        WinnerMapper localWinnerMapper = localSqlSession2.getMapper(WinnerMapper.class);
        localWinnerMapper.save(Winner.builder().id(3).userId(3).winner(WinnerType.LOSER).createdAt(Instant.now()).build());
        localWinnerMapper.save(Winner.builder().id(4).userId(4).winner(WinnerType.LOSER).createdAt(Instant.now()).build());
        transactionManager.commit(status2);
    ...
    public void serializable_read_insert() {
        ...
        SqlSession localSqlSession2 = sqlSessionFactory.openSession();
        WinnerMapper localWinnerMapper = localSqlSession2.getMapper(WinnerMapper.class);
        localWinnerMapper.save(Winner.builder().id(5).userId(5).winner(WinnerType.WINNER).createdAt(Instant.now()).build());
        localWinnerMapper.save(Winner.builder().id(6).userId(6).winner(WinnerType.WINNER).createdAt(Instant.now()).build());
        transactionManager.commit(status2);
