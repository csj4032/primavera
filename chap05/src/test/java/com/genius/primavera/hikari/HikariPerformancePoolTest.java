...
class HikariPerformancePoolTest {
    ...
    void measureMaximumConcurrencyStressTest() {
        ...
        log.info("=== 최대 동시성 스트레스 테스트 시작: {}개 쓰레드 × {}개 쿼리 ===", 
        ...
        // 결과 분석
        ThreadResult[] results = java.util.Arrays.stream(futures)
        ...
        // 성능 어설션 - 높은 처리량 기대
        Assertions.assertTrue(totalQueriesPerSecond > 50, 
            "성능 최우선 설정에서는 50 queries/sec 이상 처리량 기대");
    }
    ...
    void verifyMinimalConnectionCreationOverhead() {
        ...
        // 초기 상태 확인 - minimum-idle로 인해 이미 연결이 준비되어 있어야 함
        ...
        // 어설션 - 사전 생성된 연결로 인해 빠른 응답 기대
        Assertions.assertTrue(avgQueryTime < 50, 
            "사전 생성된 연결로 인해 평균 50ms 이하 응답 시간 기대");
    }
