-- chap14 Database Schema
-- JPA 관계 매핑 테스트를 위한 종합 데이터베이스 스키마
-- 이 스키마는 chap14의 JPA 관계 매핑 학습 기능을 지원합니다.
...
-- JPA 시퀀스 초기화
INSERT INTO TABLE_SEQ (SEQ_NAME, SEQ_COUNT) VALUES 
('FAMILY', 0),
('ITEM', 0), 
('CONTACT', 0)
...
-- 데이터 검증 쿼리 (주석 처리)
-- ===========================================
-- 생성된 데이터를 확인하려면 아래 쿼리들의 주석을 해제하세요:

-- SELECT 'OneToOne 관계 테스트' as TEST_CATEGORY;
-- SELECT M.NAME, A.STREET, A.CITY FROM MEMBERS M LEFT JOIN ADDRESSES A ON M.ID = A.MEMBER_ID;

-- SELECT 'OneToMany 관계 테스트' as TEST_CATEGORY;  
-- SELECT C.NAME, COUNT(CT.ID) as CONTACT_COUNT FROM CUSTOMERS C LEFT JOIN CONTACTS CT ON C.ID = CT.CUSTOMER_ID GROUP BY C.ID;

-- SELECT 'ManyToOne 관계 테스트' as TEST_CATEGORY;
-- SELECT D.NAME as DEPT, COUNT(E.ID) as EMP_COUNT FROM DEPARTMENTS D LEFT JOIN EMPLOYEES E ON D.ID = E.DEPARTMENT_ID GROUP BY D.ID;

-- SELECT 'ManyToMany 관계 테스트' as TEST_CATEGORY;
-- SELECT B.NAME as BUYER, S.NAME as SELLER, C.CONTRACT_NUMBER FROM CONTRACTS C JOIN BUYERS B ON C.BUYER_ID = B.ID JOIN SELLERS S ON C.SELLER_ID = S.ID;

-- SELECT '상속 계층 테스트' as TEST_CATEGORY;
-- SELECT F.NAME, 'CANIDAE' as TYPE FROM FAMILY F JOIN CANIDAE C ON F.ID = C.ID
-- UNION ALL
-- SELECT F.NAME, 'FELIDAE' as TYPE FROM FAMILY F JOIN FELIDAE FE ON F.ID = FE.ID
-- UNION ALL  
-- SELECT F.NAME, 'SCINCIDAE' as TYPE FROM FAMILY F JOIN SCINCIDAE S ON F.ID = S.ID;