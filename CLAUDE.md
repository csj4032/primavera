...
- 외부 시스템과의 통합 지점
- 보안 관련 중요 사항이나 성능상 주의가 필요한 부분
- **과도한 주석 피하기**: 명백한 코드에 주석 달지 않기
- **의미 있는 네이밍 우선**: 주석 없이도 이해할 수 있는 클래스명, 메서드명 사용
...
동적 빈 생성: 선언된 컨테이너는 고유한 이름을 가진 스프링 빈(Bean)으로 자동 등록되어 테스트 코드에서 주입하여 사용할 수 있습니다.
다중/이중화 구성: 여러 종류의 컨테이너를 동시에 사용하거나, 같은 종류의 컨테이너를 여러 개(이중화) 띄우는 구성이 가능해야 합니다.
테스트 격리: 모든 컨테이너는 테스트 클래스 단위로 생성 및 소멸되어 완벽한 데이터 격리를 보장합니다.
JUnit 생명주기 호환: JUnit 5의 TestInstance Lifecycle이 PER_CLASS이든 PER_METHOD이든 관계없이 컨테이너의 생명주기는 클래스 단위로 일관되게 동작해야 합니다.
JUnit 병렬처리 : JUnit 5의 Execution Mode SAME_THREAD, CONCURRENT를 사용하여 병렬 테스트 실행 시에도 컨테이너가 충돌 없이 독립적으로 동작해야 합니다.
테스트 우선 개발 : 최대한 단계별로 테스트를 진행하고 의도하지 않은 작동 및 오류를 조기에 수정한 뒤 다음 단계로 넘어갈 수 있도록 합니다.
소프트웨어 아키텍처 : SOLID, KISS, YAGNI, DRY 원칙 및 디자인패턴(Design Pattern) 준수하여 유지보수성과 확장성을 고려한 구조로 작성합니다.
주석 처리 : 절대 주석을 남기지 않습니다. 코드로 충분히 이해할 수 있도록 작성합니다.