...
public class ConfigurationBindingTest {
    ...
    void testConfigurationPropertyBinding() {
        ...
        assertTrue(cacheInfo.spec() instanceof RedisContainerSpec, "Cache spec should be RedisContainerSpec");
        RedisContainerSpec cacheSpec = (RedisContainerSpec) cacheInfo.spec();
        assertEquals("redis_password", cacheSpec.getPassword(), "Cache should use configured password");
    ...
    void testEnvironmentVariables() {
        ...
        assertEquals("utf8mb4", primaryEnv.get("MYSQL_CHARSET"), "Primary DB should have configured charset");
        assertEquals("utf8mb4_unicode_ci", primaryEnv.get("MYSQL_COLLATION"), "Primary DB should have configured collation");

        Map<String, String> cacheEnv = cacheInfo.spec().getEnvironment();
        assertNotNull(cacheEnv, "Cache environment should not be null");
        assertEquals("256mb", cacheEnv.get("REDIS_MAXMEMORY"), "Cache should have configured max memory");
        assertEquals("allkeys-lru", cacheEnv.get("REDIS_MAXMEMORY_POLICY"), "Cache should have configured eviction policy");
