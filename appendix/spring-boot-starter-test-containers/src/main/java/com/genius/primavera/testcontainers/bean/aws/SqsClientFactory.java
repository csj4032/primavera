...
@Slf4j
public class SqsClientFactory extends AwsServiceClientFactory {

    private static final String SQS_CLIENT_CLASS = "software.amazon.awssdk.services.sqs.SqsClient";
    ...
    public Object createClient(LocalStackContainer container) {
        if (!isAvailable()) {
            throw new IllegalStateException("AWS SQS SDK is not available in classpath. AWS SDK v2  dependency needs to be added.");
        }

        try {
            Class<?> sqsClientClass = Class.forName(SQS_CLIENT_CLASS);
            ...
            String endpointUrl = getEndpointUrl(container, LocalStackContainer.Service.SQS);
            ...
            log.info(" SqsClient created successfully. Endpoint: {}", endpointUrl);
            return sqsClient;

        } catch (Exception e) {
            log.error("SqsClient creation failed with error", e);
    ...
    public LocalStackContainerSpec.AwsService getSupportedService() {
        return LocalStackContainerSpec.AwsService.SQS;
    }
