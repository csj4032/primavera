...
public class OrderServiceImpl implements OrderService {
    ...
    public Mono<Order> createOrder(CreateOrderRequest request) {
        return Mono.fromCallable(() -> {
                    ...
                    BigDecimal totalAmount = request.getItems().stream().map(item -> item.getUnitPrice().multiply(BigDecimal.valueOf(item.getQuantity()))).reduce(BigDecimal.ZERO, BigDecimal::add);
                ...
                .flatMap(savedOrder -> {
                    ...
                    return eventPublisher.publishOrderCreatedEvent(event).thenReturn(savedOrder);
                })
                .doOnSuccess(order -> log.info("test creation completed: orderId={}, customerId={}, totalAmount={}", order.getOrderId(), order.getCustomerId(), order.getTotalAmount()))
                .doOnError(error -> log.error("test creation failure: customerId={}, error={}", request.getCustomerId(), error.getMessage()));
    }
    ...
    public Mono<Order> cancelOrder(String orderId, String reason) {
        return orderRepository.findByOrderId(orderId)
                ...
                .flatMap(cancelledOrder -> eventPublisher.publishOrderCancelledEvent(orderId, reason).thenReturn(cancelledOrder))
    ...
    public Mono<Order> getOrderById(String orderId) {
        return orderRepository.findByOrderId(orderId).switchIfEmpty(Mono.error(new RuntimeException("test should file: " + orderId)));
    }
